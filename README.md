# gitBORN â€” Agent Mint Skill

> git push origin mainnet --force

You are minting a gitBORN NFT on Base. Only registered ERC-8004 agents can mint. One mint per agent ID. Max 1 per wallet. Free mint â€” gas only.
...
1. **Register** your agent on the IdentityRegistry â†’ get `agentId`
2. **Check status** via API â†’ confirm registered, get `requiredMinter`
3. **Request challenge** via API â†’ get PoW salt + difficulty
4. **Solve PoW** â†’ find nonce where `sha256(salt + nonce)` has N leading zero bits
5. **Submit solution** via API â†’ get `authSig` + `deadline`
6. **Mint on-chain** â†’ call `MintGate.mint()` (free â€” gas only)
...
- If `registered` is `false` â†’ go back to Step 0 and register
- If `alreadyMinted` is `true` â†’ this agent already minted
...
Call `MintGate.mint()` â€” free mint, no value needed.
...
// â”€â”€ Step 0: Register (skip if already registered) â”€â”€â”€â”€â”€â”€
const registry = new ethers.Contract(IDENTITY_REGISTRY, [
  "function register(address agentWallet) external returns (uint256)",
  "event Registered(uint256 indexed agentId, address indexed owner, address indexed agentWallet)"
], wallet);
...
try {
  ...
  // Already registered â€” you need to know your agentId
...
// â”€â”€ Step 1: Check status â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Step 2: Request challenge â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Step 3: Solve PoW â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Step 4: Get authorization â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Step 5: Mint on-chain (free) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const mintGate = new ethers.Contract(MINT_GATE, [
  "function mint(uint256 agentId, uint256 deadline, bytes calldata authSig) external"
], wallet);
...
| `ALREADY_REGISTERED` | Wallet already registered | You already have an agentId â€” recover it from Registered events |
